...
def create_ssdb_feature_group(bounds):
    """Create SSDB feature group with caching - FIXED VERSION."""
    ...
    if bounds_key in st.session_state.cached_ssdb_markers:
        cached_data = st.session_state.cached_ssdb_markers[bounds_key]
        if DEBUG_PRINT:
            print(f"****INFO create_ssdb_feature_group Found cached data for bounds: {bounds_key}")
        
        if cached_data is None:
            return None
        
        # Check if cached data is old format (FeatureGroup) or new format (list of dicts)
        if hasattr(cached_data, '_children'):  # Old format - it's a FeatureGroup
            # if DEBUG_PRINT:
            #     print(f"****INFO create_ssdb_feature_group Old cache format detected, clearing cache for: {bounds_key}")
            # Remove old format from cache and continue to recreate
            del st.session_state.cached_ssdb_markers[bounds_key]
        else:  # New format - it's a list of marker data
            # if DEBUG_PRINT:
            #     print(f"****INFO create_ssdb_feature_group Using cached SSDB marker data for bounds: {bounds_key}")
            
            # ALWAYS create a fresh feature group, even with cached data
            fg_ssdb = folium.FeatureGroup(name="SSDB Locations", show=True)
            
            # Re-create markers from cached data
            markers_added = 0
            for marker_data in cached_data:
                try:
                    marker = folium.CircleMarker(
                        location=[marker_data['lat'], marker_data['lng']],
                        radius=5,
                        tooltip=marker_data['store_name'],
                        color="blue",
                        fillColor="lightblue",
                        fillOpacity=0.7,
                        weight=2,
                    ...
                    if DEBUG_PRINT:
                        print(f'!!!!WARNING create_ssdb_feature_group Could not recreate cached marker: {str(e)}')
            
            # if DEBUG_PRINT:
            #     print(f"****INFO create_ssdb_feature_group Recreated {markers_added} SSDB markers from cache")
            return fg_ssdb
    ...
    try:
        ...
        # Store marker data for caching (not the feature group object)
        cached_marker_data = []
        markers_added = 0
        ...
        for idx, row in gdf_view.iterrows():
            try:
                ...
                # Store the marker data for caching
                cached_marker_data.append({
                    'lat': lat,
                    'lng': lng,
        ...
        if markers_added > 0:
            # Cache the marker DATA (not the feature group object)
            st.session_state.cached_ssdb_markers[bounds_key] = cached_marker_data
            # if DEBUG_PRINT:
    ...
    def __init__(self):
        ...
        if "map_bounds" not in st.session_state:
            st.session_state.map_bounds = {
                "_southWest": {"lat": 50.0, "lng": -0.01},
                "_northEast": {"lat": 51.0, "lng": 0.01}
            }
        
        if 'search_locations_df' not in st.session_state:
            st.session_state.search_locations_df = pd.DataFrame()
    ...
    def _handle_map_click(self, map_data):
        ...
        if current_obj_click and obj_click_changed:
            if "lat" in current_obj_click and "lng" in current_obj_click:
                ...
                if DEBUG_PRINT:
                    print(f'****INFO Object click detected: {new_location} with tooltip: {tooltip_text}')
            ...
            if "lat" in current_map_click and "lng" in current_map_click:
                ...
                if DEBUG_PRINT:
                    print(f'****INFO Map click detected: {new_location}')
        ...
        if new_location and st.session_state.get('clicked_location') != new_location:
            ...
            if DEBUG_PRINT:
                print(f'****INFO Setting new clicked location: {new_location}')
            return True
        ...
        with col_2:
            ...
            if map_data and map_data.get("bounds"):
                ...
                if st.session_state.get('last_bounds_key') != current_bounds_key:
                    ...
                    if DEBUG_PRINT:
                        print(f"****INFO Bounds updated: {current_bounds_key}")
        ...
        with col_1:
            ...
            if not st.session_state.search_locations_df.empty:
                if st.button("Process Locations", type="primary"):
                    with st.spinner(text="Processing selected locations..."):
...
def debug_print_current_map_settings():
    ...
    # src_locations selecte
    print(f"Source locations selected: {st.session_state.get('src_locations_selected', 'Not set')}")
